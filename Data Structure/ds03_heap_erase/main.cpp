  ...
  public:
    ...
    void erase(const T& v) {
      ...
      for (; index < mSize; index++) {
        if (mData[index] == v) {found = true; break;}
      }
      //std::cout << "this - > " << mData[index] << "\n";
      if (!found) {return;}
      if (index == mSize-1) mData[index] = mData[mSize-2];
      else mData[index] = mData[mSize-1];
      //std::cout << "this - > " << mData[mSize-1] << "\n";
      mSize--;
      if (index!=0 && mData[index] > mData[(index-1)/2]) fixUp(index);
      else fixDown(index);
